...
**Goal:** 将公开 npm 包名纠正为 `@bryanhu/codexly`，升级版本并完成可验证的 npm 与 GitHub 发布。
...
**Architecture:** 仅修改公开根包身份和对应发布资料，内部 Workspace 包继续使用 `@codexly/*`，CLI 继续暴露 `codexly`；通过根 manifest 驱动打包校验与标签发布。

**Tech Stack:** pnpm、Node.js、Git、GitHub Actions、npm

## Global Constraints

- 公开 npm 包名必须精确为 `@bryanhu/codexly`，版本升级为 `0.0.3`。
...
- 根包名称更新为 `@bryanhu/codexly`、版本更新为 `0.0.3`，真实 tarball 校验拒绝其他公开包名并继续验证 `codexly` CLI 与必要产物。
...
Expected: 构建和 package verification 通过，tarball manifest 为 `@bryanhu/codexly@0.0.3`。
...
- 所有公开安装、架构、发布、稳定约束和历史计划中的包名统一为 `@bryanhu/codexly`，并记录 `0.0.3` 的包名修复。
...
Expected: 搜索无结果，安装命令均使用 `@bryanhu/codexly`。
...
Expected: 全部检查通过，`@bryanhu/codexly@0.0.3` 和 GitHub Release `v0.0.3` 均可查询。
