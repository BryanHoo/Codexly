...
- 每个活动 Task 使用独立宠物气泡并以 `AgentTask.title` 为标签；默认逐层叠放且后续层只露出底部三分之一，悬停或聚焦宠物区域时展开；后台 Task 完成后将 loading 状态替换为完成状态并持续保留，直到用户点击气泡或自行进入对应 Task；点击气泡通过应用内路由进入对应 Task；宽度随内容伸缩，最大为 `12rem` 且不得超出视口。
