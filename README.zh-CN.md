...
Codexly 桌面客户端位于 [`desktop/`](desktop/README.md)。两端独立开发、独立构建，共用本仓库的版本标签与发布流水线；发布说明见[联合发布指南](docs/releasing.md)。

## 主要功能

- 运行项目任务或临时任务，实时查看回复、命令和文件变更
- 在任务检查器中查看持久化的运行时警告，并展开完整上下文
...
Codexly 通过 `@openai/codex` 自带 Codex CLI `0.157.1`，无需单独安装。通过 `--codex-bin <path>` 或 `CODEXLY_CODEX_BIN` 指定的外部版本必须满足 `>=0.157.1,<0.158.0`。
