import type { AgentProviderConnectionStatus } from "@codexly/protocol";
...
export function ProviderConnectionPanelView({
  ...
  const stateLabel = pending
    ? t(mode === "official" && pendingOfficial ? "provider.waiting" : "provider.connecting")
    : connected
  ...
  return (
    <div className="w-full min-w-0 space-y-6">
      <section aria-label={t("provider.connection")}>
        <div className="mb-3 flex min-h-6 flex-wrap items-center justify-between gap-3">
          ...
          >
            <StatusIcon
              aria-hidden="true"
              className={cn("size-3.5 shrink-0", pending && "motion-safe:animate-spin")}
            />
            {stateLabel}
          </span>
        </div>
        <div
          ...
          className="grid w-full grid-cols-2 gap-1 rounded-control bg-control p-1"
          role="group"
        >
          {modes.map(({ value, icon: Icon }) => (
            <Button
              ...
              className={cn(
                "h-10 min-w-0 gap-2 text-body-small",
                mode === value
      ...
      {mode === "official" ? (
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="min-w-0 flex-1">
              ...
              <Button
                ...
                disabled={isBusy}
                onClick={
                  pendingOfficial ? onCancelLogin : connected ? onLogout : onStartOfficialLogin
                }
                type="button"
        ...
        <form
          ...
          onSubmit={(event) => {
            ...
            if (!isBusy && baseUrl.trim() !== "" && !hasIncompleteCustomModels(models))
              onConfigureCustom();
          }}
        >
          <section aria-labelledby={`${inputId}-credentials`}>
            <h2 className="mb-3 text-body font-semibold" id={`${inputId}-credentials`}>
              {t("provider.credentials")}
            </h2>
            <div className="divide-y divide-separator border-y border-separator">
              <div className="grid sm:grid-cols-[minmax(9rem,1fr)_minmax(0,22rem)] items-center gap-4 py-4">
                <label className="text-body-small font-medium" htmlFor={`${inputId}-url`}>
                  {t("provider.baseUrl")}
                </label>
                <Input
                  autoCapitalize="none"
              ...
              <div className="grid sm:grid-cols-[minmax(9rem,1fr)_minmax(0,22rem)] items-center gap-4 py-4">
                <label className="text-body-small font-medium" htmlFor={`${inputId}-key`}>
                  {t("provider.apiKey")}
                </label>
                <div className="relative min-w-0">
                  ...
                  <Tooltip>
                    <TooltipTrigger asChild>
                      <Button
                        aria-label={t(keyVisible ? "provider.hideKey" : "provider.showKey")}
                    ...
                    <TooltipContent>
                      {t(keyVisible ? "provider.hideKey" : "provider.showKey")}
                    </TooltipContent>
                  </Tooltip>
                </div>
              </div>
          ...
          <div className="flex justify-end border-t border-separator pt-4">
            <Button
              className="h-9 gap-2"
              disabled={isBusy || baseUrl.trim() === "" || hasIncompleteCustomModels(models)}
              type="submit"
            >
              {isBusy ? (
                <LoaderCircle aria-hidden="true" className="size-4 motion-safe:animate-spin" />
      ...
      {error === null ? null : (
        <div
          className="flex items-start justify-between gap-3 border-l-2 border-danger pl-3 text-body-small"
          role="alert"
        >
