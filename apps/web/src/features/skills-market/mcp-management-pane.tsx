import type { ConfiguredMcpServer } from "@codexly/protocol";
    ...
    <div className="skills-market-pane" role="tabpanel">
      <div className="skills-market-toolbar">
        <span className="text-body-small text-muted-foreground">
          {t("skillsMarket.mcpCount", { count: servers.data?.data.length ?? 0 })}
        </span>
        <Button
          ...
          variant="ghost"
        >
          <RefreshCw aria-hidden="true" className={servers.isFetching ? "animate-spin" : ""} />
        </Button>
      </div>
      {servers.isPending ? (
        ...
        <section className="skills-installed-group">
          <header>
            <h3>MCP</h3>
            <span>{servers.data.data.length}</span>
          </header>
          <ul>
            {servers.data.data.map((server) => (
              <li className="skills-installed-row" key={server.name}>
                <span className="skills-mcp-row__content">
                  <span aria-hidden="true" className="skills-market-glyph" data-tone="mcp">
                    <Server />
                  </span>
                ...
                <Switch.Root
                  ...
                  disabled={toggle.isPending && toggle.variables.name === server.name}
                  onCheckedChange={() => {
                    toggle.mutate(server);
                  }}
                >
