...
export function OfficialPluginSheet({
  ...
  const detail = useQuery({
    queryFn: () =>
      client.getOfficialPlugin(plugin.marketplaceName, plugin.marketplacePath, plugin.pluginName),
    queryKey: [
      "extensions",
      "plugins",
      "detail",
              ...
              <div className="official-plugin-assets">
                ...
                <section>
                  <h4>
                    ...
                    MCP <span>{data?.mcpServers.length ?? 0}</span>
                  </h4>
                ...
                {data?.hooks.length === 0 ? null : (
                  <section>
                    <h4>
                      <Workflow aria-hidden="true" />
                      Hooks <span>{data?.hooks.length ?? 0}</span>
                    </h4>
                    {data?.hooks.map((hook) => (
                      <div className="official-plugin-asset" key={hook}>
                        <strong>{hook}</strong>
                      </div>
                    ))}
                  </section>
        ...
        <SheetFooter className="official-plugin-sheet__footer">
          {plugin.installed ? (
            <Button
              ...
              variant="destructive"
            >
              {uninstalling ? <LoaderCircle aria-hidden="true" className="animate-spin" /> : null}
