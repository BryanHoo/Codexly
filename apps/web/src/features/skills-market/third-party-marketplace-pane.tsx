import type { ClawhubSkillPage, ClawhubSkillSummary, SkillInstallScope } from "@codexly/protocol";
import {
  useInfiniteQuery,
  useMutation,
  useQuery,
...
function useDebouncedValue(value: string): string {
  ...
  useEffect(() => {
    // 输入稳定后再访问 ClawHub，降低远端请求频率和列表渲染抖动。
...
function SkillCard({
  skill,
  onOpen,
}: Readonly<{ onOpen: (skill: ClawhubSkillSummary) => void; skill: ClawhubSkillSummary }>) {
  return (
    <button
    ...
    >
      <span className="skills-market-card__topline">
        <span className="skills-market-glyph" data-tone="marketplace" aria-hidden="true">
          <Blocks />
        </span>
...
export function ThirdPartyMarketplacePane({
  projectId,
  rootPath,
}: Readonly<{ projectId?: string; rootPath?: string }>) {
  ...
  const installed = useQuery({
    ...
    queryKey: ["extensions", "skills"],
  });
  const market = useInfiniteQuery({
    getNextPageParam: (page: ClawhubSkillPage) => page.nextCursor ?? undefined,
    initialPageParam: null as string | null,
    queryFn: ({ pageParam }) => client.listClawhubSkills(deferredQuery, pageParam, sort),
    queryKey: ["extensions", "marketplace", "clawhub", deferredQuery, sort],
    staleTime: 2 * 60_000,
  });
