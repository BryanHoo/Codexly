...
import type { ScheduledTaskPreview, ScheduledTaskSchedule } from "@codexly/protocol";
...
export function useSchedulePreview(
  schedule: ScheduledTaskSchedule | undefined,
  preview: PreviewSchedule,
) {
  const [result, setResult] =
    useState<Readonly<{ key: string; dates?: readonly number[]; failed?: boolean }>>();
  const [attempt, setAttempt] = useState(0);
  const inFlight = useRef<Promise<void> | undefined>(undefined);
  // 只用调度内容作依赖；名称、提示词与父组件重绘不会重发预览请求。
    ...
    const timer = setTimeout(() => {
      const request = async () => {
        ...
        const work = Promise.resolve()
          ...
          .then(
            (response) => {
              if (!cancelled) setResult({ key, dates: response.dates });
            },
            () => {
