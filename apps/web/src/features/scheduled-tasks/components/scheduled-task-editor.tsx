        ...
        <section className="scheduled-task-section">
          ...
          <div className="scheduled-task-fields">
            ...
            <div className="scheduled-task-info">
              <span>{t("scheduledTasks.runIn")}</span>
              <span>{t("scheduledTasks.newChat")}</span>
            </div>
          </div>
        </section>
        <section className="scheduled-task-section">
          ...
          <>
            <div className="scheduled-task-runs">
              <h3>
                ...
                <span>{props.task.runs.length}</span>
              </h3>
              {props.task.runs.length === 0 ? (
                <p>{t("scheduledTasks.noRuns")}</p>
              ) : (
                props.task.runs.toReversed().map((run) => (
                  <div className="scheduled-task-run" data-status={run.status} key={run.id}>
                    ...
                    {run.taskId === null ? (
                      ...
                      <Button
                        ...
                        disabled={props.openingRun}
                        onClick={() => {
                          if (props.task !== undefined && run.taskId !== null) {
                            props.onOpenRun(props.task.projectId, run.taskId);
                          }
                        }}
                        size="icon-sm"
                        variant="ghost"
