...
export function ScheduledTaskMenu({
  task,
  onDelete,
  onEnabledChange,
}: Readonly<{
    ...
    <>
      <DropdownMenu modal={false}>
        <DropdownMenuTrigger asChild>
          <Button
            aria-label={t("scheduledTasks.taskActions", { name: task.name })}
            className="scheduled-task-row__menu"
            size="icon-toolbar"
            variant="ghost"
          >
        ...
        <DropdownMenuContent align="end" sideOffset={4} className="scheduled-task-menu">
          <DropdownMenuItem
            ...
            onSelect={() => {
              onEnabledChange(task.id, !task.enabled);
            }}
          >
