...
export function ScheduledTaskPreview({
  ...
  const zoneName = useMemo(
    () =>
      new Intl.DateTimeFormat(language, { timeZone: timezone, timeZoneName: "longGeneric" })
  ...
  let rule = t("scheduledTasks.summaryEvery", {
    interval,
    unit: t(`scheduledTasks.units.${frequency}`),
  });
  if (schedule.preset === "weekdays" || schedule.preset === "weekends")
    rule = t(`scheduledTasks.${schedule.preset}`);
  else if (frequency === "WEEKLY")
    rule += ` · ${SCHEDULE_WEEKDAYS.filter((day) => schedule.weekdays.includes(day))
      .map((day) => weekdays[SCHEDULE_WEEKDAYS.indexOf(day)])
      .join("、")}`;
  if (frequency === "MONTHLY") {
    if (schedule.monthMode === "date") {
      ...
      rule += ` · ${schedule.ordinals.map((n) => t(`scheduledTasks.ordinals.${String(n)}`)).join("、")} ${weekdays[SCHEDULE_WEEKDAYS.indexOf(schedule.monthWeekday)] ?? ""}`;
  }
  if (frequency === "YEARLY")
    rule += ` · ${t("scheduledTasks.summaryYearDate", { month: schedule.yearMonth, day: schedule.yearDay })}`;
  const summary =
    schedule.preset === "once"
      ? t("scheduledTasks.once")
      : frequency === "HOURLY"
        ? rule
        : t("scheduledTasks.summaryAt", { rule, time: schedule.time });
  return (
    <div className="scheduled-task-preview" aria-live="polite" aria-busy={pending}>
      <p className="scheduled-task-preview__summary">{summary}</p>
      <p className="scheduled-task-preview__meta">
        {zoneName}
        {schedule.preset !== "once"
          ? ` · ${t("scheduledTasks.summaryFrom", { date: schedule.startDate })}${frequency === "HOURLY" ? ` ${schedule.time}` : ""}`
          : ""}
      </p>
      {schedule.preset !== "once" && schedule.endMode !== "never" ? (
        <p className="scheduled-task-preview__meta">
          {schedule.endMode === "date"
            ? t("scheduledTasks.summaryUntil", { date: schedule.endDate })
            : t("scheduledTasks.summaryCount", { count: schedule.count })}
        </p>
      ) : null}
