...
describe("visual recurrence", () => {
  it("builds and restores weekends independently of custom weekday selections", () => {
    const schedule = draftToSchedule(
      { ...draft(), preset: "weekends", weekdays: [], endMode: "count", count: 6 },
      "Asia/Shanghai",
      now,
    );
    if (schedule === undefined) throw new Error("Expected schedule");
    expect(schedule).toMatchObject({
      rrule: "RRULE:FREQ=WEEKLY;WKST=MO;BYDAY=SA,SU;BYHOUR=9;BYMINUTE=15;BYSECOND=0;COUNT=6",
    });
    expect(scheduleToDraft(schedule)).toMatchObject({
      preset: "weekends",
      weekdays: ["SA", "SU"],
      count: 6,
    });
  ...
  it("builds anchored biweekly rules with multiple weekdays and a finite count", () => {
    expect(
      ...
      rrule:
        "RRULE:FREQ=WEEKLY;INTERVAL=2;WKST=MO;BYDAY=MO,WE,FR;BYHOUR=9;BYMINUTE=15;BYSECOND=0;COUNT=6",
    });
  });
  it("round trips monthly ordinals, month end, hourly and yearly schedules", () => {
    ...
    for (const value of variants) {
      ...
      expect(
        draftToSchedule(scheduleToDraft(schedule), "America/New_York", now + 86_400_000),
      ).toEqual(schedule);
    }
  });
  it("includes the entire end date in the task timezone", () => {
    const schedule = draftToSchedule(
      { ...draft(), preset: "daily", endMode: "date", endDate: "2030-01-10" },
      "Asia/Shanghai",
      now,
    );
    if (schedule?.type !== "rrule") throw new Error("Expected recurrence");
    expect(schedule.rrule).toContain("UNTIL=20300110T155959Z");
  ...
  it("restores saved anchors and wall time even when the computer timezone differs", () => {
    const schedule = {
      ...
      startAtUnixMs: Date.UTC(2030, 0, 2, 14, 15),
    ...
    expect(draftToSchedule(restored, schedule.timezone, now + 999999999)).toMatchObject({
  ...
  it("rejects nonexistent DST wall times instead of shifting them silently", () => {
    expect(
      draftToSchedule(
        { ...draft(), preset: "daily", startDate: "2030-03-10", time: "02:30" },
        "America/New_York",
        now,
      ),
    ).toBeUndefined();
  ...
  it("keeps unsupported constraints from being silently overwritten", () => {
    for (const rrule of [
      ...
      "FREQ=MONTHLY;BYDAY=2WE,4FR",
    ]) {
      const restored = scheduleToDraft({
        type: "rrule",
        timezone: "UTC",
        startAtUnixMs: now,
        rrule,
  ...
  it("formats dates using the application language", () => {
    for (const language of ["en", "zh-CN"])
      expect(formatScheduledTime(now, language)).toBe(
