import { CodexlyHttpError, type CodexlyClient } from "@codexly/client";

export async function readScheduledTaskRun(
  client: Pick<CodexlyClient, "readTask" | "listTasks">,
  projectId: string,
  taskId: string,
) {
  let response;
  try {
    ...
    if (error instanceof CodexlyHttpError && (error.status === 404 || error.status === 410))
      return null;
    const message = error instanceof Error ? error.message : typeof error === "string" ? error : "";
    if (
      /thread.*(?:not loaded|not found|does not exist|deleted|archived)|no rollout found/iu.test(
        message,
      )
    )
      return null;
