      ...
      <div className="space-y-6">
        <SettingsGroup title={t("agent.defaults")}>
          <SettingsField
            ...
            description={t("general.approvalDescription")}
          >
            <SettingsSelect
              aria-label={t("fields.approvalPolicy")}
              value={deriveApprovalMode(settings)}
              onChange={(event) => {
                onChange(applyApprovalMode(settings, event.currentTarget.value as ApprovalMode));
              }}
            >
          ...
          <SettingsField label={t("fields.sandbox")} description={t("general.sandboxDescription")}>
            <SettingsSelect
              ...
              onChange={(event) => {
                onChange({
                  ...settings,
                  sandboxMode: event.currentTarget.value as AgentGlobalSettings["sandboxMode"],
                });
              }}
            >
          ...
          <AgentPreferencesFields client={client} />
        </SettingsGroup>
          ...
          <SettingsField label={t("fields.model")}>
            <ModelSelect
              ...
              onChange={(modelId) => {
                onChange({
                  ...settings,
                  ...resolveGlobalSettingsModel(models, modelId, settings.reasoningEffort),
                });
              }}
            />
          </SettingsField>
          <SettingsField label={t("fields.reasoningEffort")}>
            <ReasoningSelect
              ...
              onChange={(reasoningEffort) => {
                onChange({ ...settings, reasoningEffort });
              }}
            />
          </SettingsField>
        </SettingsGroup>
