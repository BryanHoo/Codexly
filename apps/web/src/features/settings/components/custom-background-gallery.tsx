...
import {
  useCustomWallpaperSource,
  WallpaperAction,
  WallpaperImageDialog,
} from "./wallpaper-controls.js";
import type { WorkbenchBackgroundSettingsProps } from "./workbench-background-settings-contracts.js";
...
function CustomWallpaperTile({
  ...
  return (
    <article className="wallpaper-tile" data-selected={selected}>
      ...
      >
        {source === null ? null : (
          <img alt={image.name} src={source} loading="lazy" decoding="async" />
        )}
        {selected ? (
      ...
      <div className="wallpaper-tile-footer">
        ...
        <div className="flex shrink-0">
          <WallpaperAction label={t("wallpaper.view", { name: image.name })} onClick={onPreview}>
            <Maximize2 aria-hidden="true" />
          </WallpaperAction>
          <WallpaperAction
            label={t("background.deleteImage", { name: image.name })}
            onClick={onRemove}
            disabled={disabled}
          >
...
function CustomWallpaperDialog({
  image,
  onClose,
}: Readonly<{ image: CustomBackgroundImage; onClose: () => void }>) {
  const source = useCustomWallpaperSource(image);
  ...
  return (
    <section>
      <div className="wallpaper-section-heading">
        <h2>
          {t("wallpaper.custom")}
          <span className="wallpaper-heading-meta">{customImages.length}</span>
        </h2>
        <Button disabled={disabled} onClick={upload} type="button" variant="outline">
      ...
      {loadError ? (
        ...
        <button
          className="wallpaper-upload-empty"
          disabled={disabled}
          onClick={upload}
          type="button"
        ...
        >
          ...
          <span>{t("wallpaper.empty")}</span>
          <span className="text-brand">{t("wallpaper.upload")}</span>
        </button>
      ) : (
