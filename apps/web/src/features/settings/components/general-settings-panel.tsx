import type { AgentGlobalSettings, ProjectOpenApp } from "@codexly/protocol";
    ...
    <section id="settings-panel-appearance">
      ...
      <div className="space-y-6">
        <SettingsGroup title={t("general.preferences")}>
          ...
          >
            <div
              className="grid w-40 grid-cols-3 rounded-control bg-control p-0.5"
              role="group"
                ...
                <Tooltip key={value}>
                  <TooltipTrigger asChild>
                    <Button
                      ...
                      className={`h-7 min-w-0 px-2 ${theme === value ? "bg-raised text-foreground shadow-control" : "text-muted-foreground"}`}
                      variant="ghost"
                      type="button"
                      onClick={() => {
                        onThemeChange(value);
        ...
        <SettingsGroup title={t("general.editor")}>
          <SettingsField
            label={t("fields.followUpMessages")}
            description={t("general.followUpDescription")}
          >
            <div
              className="grid w-40 grid-cols-2 rounded-control bg-control p-0.5"
              role="group"
            ...
            >
              {(["queue", "steer"] as const).map((value) => (
                <Button
                  ...
                  className={`h-7 px-3 text-body-small ${settings.followUpBehavior === value ? "bg-raised text-foreground shadow-control" : "text-muted-foreground"}`}
                  variant="ghost"
                  type="button"
                  onClick={() => {
        ...
        <SettingsGroup title={t("appearance.notifications")}>
          <SettingsField
            label={t("general.taskNotifications")}
            description={t("general.notificationsDescription")}
          >
            <Button
              role="switch"
              aria-label={t("general.taskNotifications")}
              aria-checked={notificationsEnabled}
              className={`h-6 w-10 rounded-pill p-0.5 ${notificationsEnabled ? "bg-brand hover:bg-brand" : "bg-control-hover"}`}
              variant="ghost"
              type="button"
              onClick={() => {
            ...
            >
              <span
                aria-hidden="true"
                className={`block size-5 rounded-pill bg-brand-contrast shadow-control transition-transform motion-reduce:transition-none ${notificationsEnabled ? "translate-x-2" : "-translate-x-2"}`}
              />
            </Button>
          </SettingsField>
        </SettingsGroup>
