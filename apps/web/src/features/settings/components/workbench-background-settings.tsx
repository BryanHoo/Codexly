...
const BackgroundImagePicker = lazy(() =>
  import("./background-image-picker.js").then((module) => ({
    default: module.BackgroundImagePicker,
  })),
);
  ...
  return (
    <div className="wallpaper-settings">
        ...
        <div className="background-mode-control" role="group" aria-label={t("wallpaper.source")}>
          {backgroundModes.map(({ ariaKey, labelKey, value }) => (
            <Button
              ...
              onClick={() => {
                onPreferenceChange({ ...preference, mode: value });
              }}
              type="button"
              variant="ghost"
            >
        ...
        <div className="background-details">
          <div className="background-summary">
            {preference.mode === "bing" ? (
              <BingBackgroundPreview preference={preference} />
            ) : (
              <CustomBackgroundPreview
                image={customImages.find((image) => image.id === preference.selectedCustomImageId)}
              />
            )}
            <Button
              variant="outline"
      ...
      {pickerOpen ? (
        <Suspense
          fallback={
            <span role="status" className="px-4 text-label text-muted-foreground">
              {t("wallpaper.loading")}
            </span>
          }
        >
