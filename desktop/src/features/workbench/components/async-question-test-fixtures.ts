...
import { createTaskItemKey, createTaskItemStore } from "../../conversation/runtime/task-store-core.js";
...
export function questionItem(id: string, title = id): Extract<AgentItem, { type: "message" }> {
  return { id, type: "message", role: "assistant", text: title,
...
export function questionTask(items: AgentItem[]) {
  ...
  store.setState({
    ...
    itemStoresByKey: new Map(items.map((item) => [createTaskItemKey("turn-a", item.id), createTaskItemStore(item)])),
  });
  return store;
}
