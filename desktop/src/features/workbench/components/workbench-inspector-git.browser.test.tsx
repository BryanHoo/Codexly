...
it.for([
  ...
  "项目页等待真实统计，审核打开标签，提交切换变更，上下文移除变更模块（$width / $theme）",
  async ({ width, theme }) => {
    document.documentElement.style.colorScheme = theme;
    await page.viewport(width, 720);
    vi.spyOn(nativeClient, "listProjectFiles").mockResolvedValue({
      entries: [{ path: "main.ts", type: "file" }],
      path: null,
    });
          ...
          <TooltipProvider>
            ...
            <div style={{ height: 640, width: 360, display: "flex" }}>
              <WorkbenchInspector
                ...
                gitStatus={status}
                gitStatusDetails={loaded ? details : undefined}
                onCommitChanges={() => setTab("changes")}
                onReviewFileChanges={(changes) => {
                  review(changes);
                  setDocuments([{ id: "review:test", kind: "review", changes }]);
                  setTab(documentTabId("review:test"));
                }}
              />
            </div>
          </TooltipProvider>
    ...
    const commitRect = screen
      .getByRole("button", { name: /提交.*变更/u })
      .element()
      .getBoundingClientRect();
    ...
    await expect
      .element(screen.getByRole("tab", { name: "审核", exact: true }))
    ...
    await expect
      .element(screen.getByRole("tab", { name: "变更", exact: true }))
      .toHaveAttribute("aria-selected", "true");
