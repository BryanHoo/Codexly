import { expect, type Page } from "@playwright/test";

// 集中维护该组 Playwright 流程使用的页面操作。
export function getComposerModelSelector(page: Page) {
  return page.getByRole("button", { name: /^选择模型：/u });
}

export function getComposerReasoningSelector(page: Page) {
  return page.getByRole("button", { name: /^选择思考量：/u });
}

export async function expectComposerSelection(page: Page, model: string, effort: string) {
  await expect(getComposerModelSelector(page)).toHaveAccessibleName(`选择模型：${model}`);
  await expect(getComposerReasoningSelector(page)).toHaveAccessibleName(`选择思考量：${effort}`);
}

export async function selectComposerModel(page: Page, modelName: string): Promise<void> {
  await getComposerModelSelector(page).click();
  await page
    .getByRole("menu", { name: "选择模型" })
    .getByRole("menuitemradio", { name: new RegExp(modelName, "u") })
    .click();
}

export async function selectComposerReasoning(page: Page, effortName: string): Promise<void> {
  await getComposerReasoningSelector(page).click();
  await page
    .getByRole("menu", { name: "选择思考量" })
    .getByRole("menuitemradio", { name: new RegExp(`^${effortName}`, "u") })
    .click();
}
