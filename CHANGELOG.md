...
- 在任务右侧检查器中展示可展开的运行时警告，保留完整上下文并在任务恢复后继续显示。
...
- 支持在局域网访问时下载任务文件。
- 在 AI 回复中渲染 Mermaid 图表，并使用严格安全解析。
- 自动发现自定义 Provider 模型目录，完善在线、CLI 与持久化目录的回退。

### 优化

- 升级内置 Codex 至 `0.156.0`，刷新 App Server Schema 基线并适配远程文件图片输入。
- 为缺少推理元数据的自定义模型提供基础思考档位，并统一推理摘要展示与异步错误提示。

### 修复

- 修复异步问题在任务终态或快照刷新后错误恢复，以及历史回答缺失的问题。
- 修复流式恢复后的运行时警告，以及已完成推理摘要仍持续展开的问题。
