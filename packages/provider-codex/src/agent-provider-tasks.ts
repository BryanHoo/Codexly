    ...
    try {
      ...
      if (!isProjectThread(thread, this.project)) {
        ...
        if (
          ...
          // Codex 0.152 的内存快照会在首条消息落盘前暂时省略已分配的 projectId。
          projectOwnershipVerified = true;
          this.promotePendingServerRequests(taskId);
          return createUnmaterializedTaskSnapshot(unmaterializedTask);
        }
      ...
      // 先恢复 Thread 级元数据，再同步映射历史，确保跨进程仍使用相同授权 ID。
      await this.threadAttachments?.prepare(taskId);
      ...
      await this.threadAttachments?.flush(taskId);
