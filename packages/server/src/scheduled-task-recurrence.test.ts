...
describe("scheduled task recurrence execution", () => {
  it("seeks old hourly anchors while preserving the interval phase", () => {
  ...
  it("keeps local time across DST and shares the preview with execution", () => {
    const schedule = recurring("FREQ=DAILY;BYHOUR=9;BYMINUTE=15;BYSECOND=0;COUNT=4");
    const dates = previewScheduledTask(schedule, Date.UTC(2030, 2, 8));
    ...
    expect(resolveNextScheduledRun(schedule, Date.UTC(2030, 2, 9, 14, 15))).toBe(dates[2]);
    expect(previewScheduledTask(schedule, Date.UTC(2030, 2, 11, 13, 15))).toEqual([]);
  });

  it("skips nonexistent wall times without consuming COUNT", () => {
    const schedule = recurring(
      "FREQ=DAILY;BYHOUR=2;BYMINUTE=30;BYSECOND=0;COUNT=3",
      "America/New_York",
      Date.UTC(2030, 2, 9, 7, 30),
    );
    expect(previewScheduledTask(schedule, Date.UTC(2030, 2, 9))).toEqual([
      Date.UTC(2030, 2, 9, 7, 30),
  ...
  it("includes the entire end date in the task timezone", () => {
    ...
    expect(previewScheduledTask(schedule, Date.UTC(2030, 0, 1))).toEqual([
      Date.UTC(2030, 0, 1, 15),
  ...
  ] as const)("evaluates calendar selections: %s", (rrule, days) => {
    const dates = previewScheduledTask(
